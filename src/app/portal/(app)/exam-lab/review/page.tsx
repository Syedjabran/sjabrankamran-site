  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="font-display text-2xl text-ice">My answer scripts</h1><p className="text-sm text-dust">Your submitted topical drills, yearly papers, self-tests and proctored tests.</p></div>
      </div>
      ...
      {attempts.map((a) => {
        ...
        const mode = a.context?.kind === "test" ? "Proctored test" : a.context?.kind === "assignment" ? "Assignment" : a.mode === "drill" ? "Topical drill" : "Paper / self-test";
        return <details key={`${a.id}-${a.ts}`} className="rounded-2xl border border-white/10 bg-space/60 p-4">
          <summary className="cursor-pointer list-none">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-display text-base text-ice">{a.code || a.ref || mode}</p><p className="mt-1 text-xs text-dust">{mode} · {when(a.ts)} · <Clock3 className="inline" size={11} /> {mins(a.durationSec)}</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-cyan/30 px-2.5 py-1 font-mono text-xs text-cyan">{score}</span><span className="rounded-full border border-white/15 px-2.5 py-1 font-mono text-[10px] text-dust">{a.context?.cancelled ? "locked/cancelled" : "submitted"}</span></div></div>
          </summary>
