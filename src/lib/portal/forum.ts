...
/** `helpfulAwarded`: uids whose "helpful" mark already earned the author points — never re-awarded on re-toggle. */
export type Post = { id: string; authorId: string; authorName: string; body: string; attachments: Attachment[]; ts: number; helpful: number; helpfulBy: string[]; helpfulAwarded?: string[]; reactions?: Reactions };
...
/** Thread ids are generated by newId(); anything else must never reach a storage key. */
export function isThreadId(id: string): boolean { return /^[A-Za-z0-9_-]+$/.test(id); }

/**
 * Cache-busted read of a JSON doc in a private bucket (service role). Shared by
 * every Storage-as-DB module that does read-modify-write.
 *
 ...
 * replies appeared to do nothing). A unique cache-buster query forces a CDN
 * miss and returns the just-written object immediately (verified live).
 *
 * Returns undefined ONLY when the object genuinely does not exist (Supabase
 * answers 404, or 400 with a `not_found` body). Any other failure — 5xx,
 * network error, unparseable JSON — THROWS, so a caller can never mistake a
 * failed read for an empty doc and overwrite real data with it.
 */
export async function readStorageJson<T>(bucket: string, path: string): Promise<T | undefined> {
  ...
  if (res.ok) return JSON.parse(await res.text()) as T;
  if (res.status === 404) return undefined;
  const detail = await res.text().catch(() => "");
  if (res.status === 400 && /not[_ ]?found|does not exist/i.test(detail)) return undefined;
  throw new Error(`Storage read failed for ${bucket}/${path} (HTTP ${res.status}).`);
}

async function readJson<T>(bucket: string, path: string, fallback: T): Promise<T> {
  return (await readStorageJson<T>(bucket, path)) ?? fallback;
}
...
export async function addPost(threadId: string, input: { authorId: string; authorName: string; body: string; attachments: Attachment[] }): Promise<Post | null> {
  ...
  const post: Post = { id: newId(), authorId: input.authorId, authorName: input.authorName, body: input.body, attachments: input.attachments, ts: Date.now(), helpful: 0, helpfulBy: [], helpfulAwarded: [], reactions: emptyReactions() };
...
/**
 * Toggle a "helpful" mark on a post by a user. Returns the target post's
 * authorId only the FIRST time this user ever marks this post helpful (for
 * awarding points) — the awarded pair is persisted on the post, so toggling
 * off/on can never farm points.
 */
export async function markHelpful(threadId: string, postId: string, byUid: string): Promise<{ ok: boolean; awardedTo?: string; awardedName?: string; helpful?: number; on?: boolean }> {
  ...
  // Posts from before awards were tracked: everyone currently marking was
  // already paid once, so seed the ledger from the current marks.
  post.helpfulAwarded = post.helpfulAwarded || post.helpfulBy.filter((u) => u !== post.authorId);
  ...
  if (had) { post.helpfulBy = post.helpfulBy.filter((u) => u !== byUid); }
    ...
    post.helpfulBy.push(byUid);
    if (post.authorId !== byUid && !post.helpfulAwarded.includes(byUid)) {
      post.helpfulAwarded.push(byUid);
      awardedTo = post.authorId;
    }
  }
  post.helpful = post.helpfulBy.length;
  if (!(await writeJson(DATA, tpath(threadId), thread))) throw new Error("Could not save the helpful mark. Please retry.");
